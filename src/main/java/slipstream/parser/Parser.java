	...
	public boolean processCommand(String input, TaskList tasks, Ui ui, Storage storage) {
		...
		try {
			...
			case "mark":
				...
				break;
			case "unmark":
				if (commandAndTask.length < 2) throw new SlipstreamException("You need to specify a task number to unmark!");
				tasks.unmarkTask(commandAndTask[1], storage, ui);
				break;
			case "todo":
				if (commandAndTask.length < 2) throw new SlipstreamException("The description of your todo task can't be empty!");
				tasks.addToDo(commandAndTask[1], storage, ui);
				break;
			case "deadline":
				if (commandAndTask.length < 2) throw new SlipstreamException("Your deadline task needs a description and a deadline! (use /by)");
				tasks.addDeadline(commandAndTask[1], storage, ui);
				break;
			case "event":
				if (commandAndTask.length < 2) throw new SlipstreamException("Your event task needs a description and a time frame! (use /from and /to)");
				...
				break;
			case "delete":
				if (commandAndTask.length < 2) throw new SlipstreamException("The delete command needs a task number!");
